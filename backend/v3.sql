...
-- Assuming 'user_id' for an Administrator is 1 and they get access to all categories
...
CREATE OR REPLACE PROCEDURE Fproject.CreateNewUser(
    ...
    p_role_name VARCHAR DEFAULT 'New Employee', -- Default role
...
BEGIN
    ...
    -- Additional logic can be added here if needed, such as logging the event or sending a welcome email
...
BEGIN
    ...
    -- Logic to automatically assign the ticket (simplified: assign to first user with access)
    SELECT user_id INTO v_assigned_to FROM Fproject.user_categories WHERE category_id = p_category_id LIMIT 1;
...
-- Stored procedure to delete a ticket
CREATE OR REPLACE PROCEDURE Fproject.CloseTicket(p_ticket_id INT)
LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
    ...
    -- Update the ticket status to 'Closed' and set the completed_at timestamp
...
CALL Fproject.UpdateUser(
    1, -- Use the actual user_id obtained from the creation step
    'Test Updated', 'User Updated', 'updated.user@example.com', NULL, NULL
);
SELECT * FROM Fproject."user" WHERE id = 6;

-- Test the 'DeleteUser' stored procedure
CALL Fproject.DeleteUser(6);
SELECT * FROM Fproject."user" WHERE id = 6;
...
BEGIN;
CALL Fproject.EditCategoryAccess(6, 1, 'Read and Write');
SELECT * FROM Fproject.user_categories WHERE user_id = 6 AND category_id = 1;
ROLLBACK;
...
SELECT status_id, priority_id FROM Fproject.ticket WHERE id = 1;
...
DO $$
BEGIN
    PERFORM Fproject.CloseTicket(1);
END
$$;
